...
export const drawLine = async ({
    ...
    const { drawHeadText = true, direction = 'ltr' } = option ?? {};
    const {
        fontLevel,
    } = textData;
    ...
    for (
        let xRatio = baseXRatio, tokenCnt = 0;
        ...
        const {
            stopApplyArabicFont, applyArabicFont,
        ...
        const indent = tokenCnt === 0
            ...
            : 0;
        tokenEdge -= direction === 'rtl'
            ? (totalTokenWidth * xRatio + actualSpaceCount * spaceWidth + indent)
            : 0;
        ...
        for (let fragmentCnt = 0; fragmentCnt < fragmentList.length; fragmentCnt++) {
            ...
            /** Draw "whole words". */
                ...
                /** Read the comment in `analyzeToken` function, we repeat exactly the treatment there, the different is we actually draw the letter along the way. Remember the canvas must enable RTL direction before drawing Arabic text. */
                ...
                if (ArabicWordRegex.test(fragment)) {
                    ...
                    if (textDrawer) {
                        textDrawer({
                            ctx,
                            letter: fragment,
                            scaledEdge: currentPosition / xRatio,
                            scaledBaseline: drawLetterParameter.baseline,
                        });
                    } else {
                        ctx.fillText(
                            fragment,
                            currentPosition / xRatio,
